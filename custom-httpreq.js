...
`);
        ...
        */
        ...
        this.on("input",function(msg,nodeSend,nodeDone) {
            ...
            if (url.indexOf("://") !== -1 && url.indexOf("http") !== 0) {
                node.warn(RED._("node-red:httpin.errors.invalid-transport"));
                node.status({fill:"red",shape:"ring",text:"node-red:httpin.errors.invalid-transport"});
                nodeDone();
                return;
            }
                ...
                if (this.authType === "basic") {
                    ...
                    opts.hooks.afterResponse = [(response, retry) => {
                        if (response.statusCode === 401) {
                            ...
                            if (normalisedHeaders['x-www-authenticate']) {
                                let authHeader = buildDigestHeader(digestCreds.user,digestCreds.password, response.request.options.method, requestUrl.pathname, normalisedHeaders['x-www-authenticate'])
            ...
            got(url,opts).then(res => {
                ...
                if (node.ret !== "bin") {
                    ...
                    if (node.ret === "obj") {
                        ...
                        catch(e) { node.warn(RED._("node-red:httpin.errors.json-error")); }
                    }
                }
                node.status({});
                nodeSend(msg);
                ...
                if (err.code === 'ETIMEDOUT' || err.code === 'ESOCKETTIMEDOUT') {
                    node.error(RED._("node-red:common.notification.errors.no-response"), msg);
    ...
    RED.nodes.registerType("custom http request",HTTPRequest,{
        credentials: {
            user: {type:"text"},
